    ...
    <div class="head">
        <table><tr>
            ...
            <td style="text-align:right">
                ...
                <div class="docnum">{{ $invoice->invoice_number }}</div>
            </td>
        </tr></table>
    </div>
    <div class="metabox">
        <table><tr>
            ...
            <td style="text-align:right;line-height:1.9">
                <div><strong class="muted">Invoice date:</strong> {{ \Carbon\Carbon::parse($invoice->invoice_date)->format('d/m/Y') }}</div>
                <div><strong class="muted">Due date:</strong> {{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('d/m/Y') : '—' }}</div>
                <div><strong class="muted">Reference:</strong> {{ $invoice->quotation->quotation_number ?? ($invoice->proformaInvoice->proforma_number ?? ($invoice->sale->receipt_number ?? '—')) }}</div>
                <div><strong class="muted">Status:</strong> <span style="font-weight:800;text-transform:uppercase">{{ str_replace('_',' ',$invoice->payment_status) }}</span></div>
            </td>
        </tr></table>
    </div>
    <div class="items">
        ...
        <div class="amounts">
            <div class="row"><span class="k">Subtotal</span><span class="v">{{ number_format($invoice->subtotal,0) }}</span></div>
            @if($invoice->discount_amount>0)<div class="row"><span class="k">Discount</span><span class="v" style="color:#b33a3a">- {{ number_format($invoice->discount_amount,0) }}</span></div>@endif
            @if($invoice->tax_amount>0)<div class="row"><span class="k">VAT ({{ $invoice->tax_rate }}%)</span><span class="v">{{ number_format($invoice->tax_amount,0) }}</span></div>@endif
            <div class="line"><span class="k">TOTAL (TZS)</span><span class="v">{{ number_format($invoice->total_amount,0) }}</span></div>
            @if($invoice->paid_amount>0)
            <div class="row" style="margin-top:4px"><span class="k" style="color:#2e7d6b">Paid</span><span class="v" style="color:#2e7d6b">{{ number_format($invoice->paid_amount,0) }}</span></div>
            <div class="row"><span class="k" style="color:#b33a3a;font-weight:800">BALANCE DUE</span><span class="v" style="color:#b33a3a;font-weight:800">{{ number_format($invoice->balance_due,0) }}</span></div>
            @endif
        </div>
    </div>
    <div class="footnotes">
        <table><tr>
            <td style="width:50%;padding-right:20px">
                ...
                <div class="muted">{{ paymentMethodName($invoice->payment_method ?? '') ?: '—' }}</div>
                <div style="margin-top:8px"><span class="lbl">PAYMENT TERMS</span></div>
                <div class="muted">{{ $invoice->terms ?: 'Due on receipt' }}</div>
            </td>
            <td>
                <span class="lbl">NOTES &amp; TERMS</span>
                <div class="muted">{!! nl2br(e($invoice->notes ?: '')) !!} {!! nl2br(e($invoice->terms ?: '')) !!}</div>
            </td>
        </tr></table>
    </div>
    <div class="footer">Thank you for your business &middot; <b>{{ $shopName }}</b></div>
</div>
</body>
</html>