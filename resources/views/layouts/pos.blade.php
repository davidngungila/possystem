...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;}html,body{height:100%;}body{margin:0;font-family:'Raleway',sans-serif;background:var(--sand-50);color:var(--ink);-webkit-font-smoothing:antialiased;overflow-x:hidden;}
        ...
        @media(max-width:900px){.sidebar{transform:translateX(-100%);width:var(--sidebar-w);}.sidebar.mobile-open{transform:translateX(0);}.main{margin-left:0 !important;}.topbar{flex-wrap:nowrap;height:56px;min-height:56px;padding:0 14px;gap:8px;overflow:hidden;}.topbar .tb-ay{flex:1;min-width:0;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;font-size:13px;}.tb-right{flex:none;gap:6px;}}
        ...
        @media(max-width:640px){.view-wrap{padding:16px;}.topbar{padding:8px 14px;gap:8px;flex-wrap:nowrap;overflow:visible;}.tb-right{gap:8px;flex-wrap:nowrap;overflow:visible;}.tb-profile-text{max-width:96px;overflow:hidden;}.tb-ay{display:none;}.tb-live{display:none !important;}.tb-progress{padding:4px 9px;font-size:10.5px;gap:6px;}.tb-profile{padding-left:8px;margin-left:4px;gap:8px;}.tb-profile .sb-avatar{width:32px;height:32px;}.tb-profile-menu{right:-8px !important;left:auto !important;min-width:200px !important;max-width:92vw !important;}}
...
<span>Sales History</span>
            ...
            <div class="sb-label">Sales Documents</div>
            <a href="{{ \Illuminate\Support\Facades\Route::has('quotations.index') ? route('quotations.index') : url('/quotations') }}" class="sb-item {{ request()->routeIs('quotations*') ? 'active' : '' }}">
            ...
            <a href="{{ \Illuminate\Support\Facades\Route::has('proforma-invoices.index') ? route('proforma-invoices.index') : url('/proforma-invoices') }}" class="sb-item {{ request()->routeIs('proforma-invoices*') ? 'active' : '' }}">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
                <span>Proforma Invoices</span>
            </a>
            <a href="{{ \Illuminate\Support\Facades\Route::has('invoices.index') ? route('invoices.index') : url('/invoices') }}" class="sb-item {{ request()->routeIs('invoices*') ? 'active' : '' }}">
            ...
            <div class="tb-right">
                ...
                <div class="tb-profile-wrapper" tabindex="0" onclick="if(window.innerWidth<=900) this.classList.toggle('open')">
                    <div class="tb-profile" style="display:flex;align-items:center;gap:10px;margin-left:8px;padding-left:12px;border-left:1px solid var(--line);min-width:0;cursor:pointer">
                        @php $uPos = auth()->user(); $initialsPos = strtoupper(substr($uPos->name ?? 'C',0,1).substr(explode(' ', $uPos->name ?? 'Cashier')[1] ?? '',0,1)); $hasAvatarPos = !empty($uPos->avatar) && file_exists(public_path($uPos->avatar)); @endphp
                        <div class="sb-avatar" style="width:36px;height:36px;border-radius:50%;flex:none;background:var(--sand-100);border:1.5px solid var(--line);display:flex;align-items:center;justify-content:center;color:var(--coffee-700);overflow:hidden">
                            @if($hasAvatarPos)<img src="{{ asset($uPos->avatar) }}" style="width:100%;height:100%;object-fit:cover" alt="Profile">@else @if($initialsPos){{ $initialsPos }}@else<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>@endif @endif
                        </div>
                        <div class="tb-profile-text" style="line-height:1.2;min-width:0;overflow:hidden;">
                            <div style="font-weight:700;font-size:13px;color:var(--coffee-900);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $uPos->name ?? 'Cashier' }}</div>
                            <div class="tb-profile-email" style="font-size:11px;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $uPos->email ?? '' }}</div>
                        </div>
                    ...
                    <div class="tb-profile-menu">
                        <div class="tb-profile-menu-head">
                            <div class="sb-avatar" style="width:44px;height:44px;overflow:hidden">@if($hasAvatarPos)<img src="{{ asset($uPos->avatar) }}" style="width:100%;height:100%;object-fit:cover" alt="Profile">@else {{ $initialsPos }} @endif</div>
                            <div style="flex:1;min-width:0;overflow:hidden">
                                <div style="font-weight:800;font-size:13px;color:var(--coffee-900);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $uPos->name ?? 'Cashier' }}</div>
                                <div style="font-size:12px;color:var(--ink-soft);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $uPos->email ?? '' }}</div>
                            </div>
                        </div>
                        ...
                        <a href="{{ route('account.setting') }}" class="tb-profile-item"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 1 0-3.02 1.65 1.65 0 0 0 1-1.51V9a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V15a1.65 1.65 0 0 0 1.51 1"/></svg> Account Setting</a>
