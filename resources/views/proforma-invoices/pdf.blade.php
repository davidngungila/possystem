    ...
    <div class="head">
        <table><tr>
            ...
            <td style="text-align:right">
                <div class="doctitle">PROFORMA INVOICE</div>
                <div class="docnum">{{ $proforma->proforma_number }}</div>
            </td>
        </tr></table>
    </div>
    <div class="metabox">
        <table><tr>
            <td style="width:50%">
                ...
                <div class="muted">{{ $proforma->customer->phone ?? '' }}</div>
                <div class="muted">{{ $proforma->customer->address ?? '' }}</div>
            </td>
            <td style="text-align:right;line-height:1.9">
                <div><strong class="muted">Date:</strong> {{ \Carbon\Carbon::parse($proforma->date)->format('d/m/Y') }}</div>
                <div><strong class="muted">Valid until:</strong> {{ $proforma->valid_until ? \Carbon\Carbon::parse($proforma->valid_until)->format('d/m/Y') : '—' }}</div>
                <div><strong class="muted">Due date:</strong> {{ $proforma->due_date ? \Carbon\Carbon::parse($proforma->due_date)->format('d/m/Y') : '—' }}</div>
                <div><strong class="muted">Reference:</strong> {{ $proforma->quotation->quotation_number ?? '—' }}</div>
                <div><strong class="muted">Status:</strong> <span style="font-weight:800;text-transform:uppercase">{{ str_replace('_',' ',$proforma->status) }}</span></div>
            </td>
        </tr></table>
    </div>
    <div class="items">
        ...
        <div class="amounts">
            <div class="row"><span class="k">Subtotal</span><span class="v">{{ number_format($proforma->subtotal,0) }}</span></div>
            @if($proforma->discount_amount>0)<div class="row"><span class="k">Discount</span><span class="v" style="color:#b33a3a">- {{ number_format($proforma->discount_amount,0) }}</span></div>@endif
            @if($proforma->tax_amount>0)<div class="row"><span class="k">VAT ({{ $proforma->tax_rate }}%)</span><span class="v">{{ number_format($proforma->tax_amount,0) }}</span></div>@endif
            <div class="line"><span class="k">TOTAL (TZS)</span><span class="v">{{ number_format($proforma->total_amount,0) }}</span></div>
            @if($proforma->paid_amount>0)
            <div class="row" style="margin-top:4px"><span class="k" style="color:#2e7d6b">Paid</span><span class="v" style="color:#2e7d6b">{{ number_format($proforma->paid_amount,0) }}</span></div>
            <div class="row"><span class="k" style="color:#b33a3a;font-weight:800">BALANCE DUE</span><span class="v" style="color:#b33a3a;font-weight:800">{{ number_format($proforma->balance_due,0) }}</span></div>
            @endif
        </div>
    </div>
    <div class="footnotes">
        <table><tr>
            <td style="width:50%;padding-right:20px">
                ...
                <div class="muted">{{ $proforma->payment_terms ?: '—' }}</div>
                <div style="margin-top:8px"><span class="lbl">PAYMENT DETAILS</span></div>
                <div class="muted">{!! nl2br(e($proforma->bank_details ?: '—')) !!}</div>
            </td>
            <td>
                <span class="lbl">NOTES &amp; TERMS</span>
                <div class="muted">{!! nl2br(e($proforma->notes ?: '')) !!} {!! nl2br(e($proforma->terms ?: '')) !!}</div>
            </td>
        </tr></table>
    </div>
    <div class="footer">Provisional invoice — final invoice is issued after payment confirmation &middot; <b>{{ $shopName }}</b></div>
</div>
</body>
</html>