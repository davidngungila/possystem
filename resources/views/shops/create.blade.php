        ...
        <form method="POST" action="{{ route('shops.store') }}">
            ...
            <div class="form-grid">
                ...
                <div class="field @error('shop_type') err @enderror">
                    <label class="field-label">Shop Types * <span style="font-weight:400;color:var(--ink-soft)">— hold Ctrl/Cmd to select multiple</span></label>
                    <select name="shop_type[]" multiple required size="8" style="width:100%;padding:8px;border:1.5px solid var(--line);border-radius:10px;background:#fff;min-height:140px">
                        @php $oldTypes = old('shop_type', []); @endphp
                    ...
                    @error('shop_type.*')<span class="field-err">{{ $message }}</span>@enderror
                    <span class="field-hint">Single shop can handle multiple product types — select all that apply (e.g. General Shop + Pharmacy)</span>
                </div>
